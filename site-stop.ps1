...
foreach ($svc in $services) {
    $body = '{"query":"mutation { serviceInstanceUpdate(serviceId: \"' + $svc.id + '\", environmentId: \"' + $envId + '\", input: { numReplicas: 0 }) }"}'
    ...
    Write-Host "$($svc.name) 중지됨"
}
Write-Host "`n사이트가 꺼졌습니다." -ForegroundColor Red
