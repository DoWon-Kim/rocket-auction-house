...
const inputCls = 'w-full bg-surface border border-line hover:border-line-strong focus:border-accent/50 rounded-xl px-4 py-3 pl-11 text-sm text-fg placeholder:text-subtle focus:outline-none transition-colors'
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center">
      <div className="w-full max-w-sm">
        ...
        <div className="flex bg-surface border border-line rounded-xl p-1 mb-6">
          {(['find-id', 'forgot-pw'] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all duration-150 ${
                tab === t
                  ? 'bg-accent-tint text-accent-soft shadow-[0_0_10px_rgba(139,92,246,0.1)]'
  ...
  return (
    <div className="bg-surface border border-line rounded-2xl p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-2">가입 시 사용한 정보로 닉네임을 확인합니다.</p>
      </div>

      {/* 찾기 방법 선택 */}
      <div className="flex bg-[#0b0a0e] border border-surface-2 rounded-xl p-0.5">
        {(['email', 'phone'] as FindMethod[]).map(m => (
          <button key={m} type="button" onClick={() => { setMethod(m); setError(null) }}
            className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              method === m ? 'bg-accent-tint text-accent-fg' : 'text-subtle hover:text-muted'
            }`}>
      ...
      {!result ? (
        <form onSubmit={submit} className="space-y-4">
          {method === 'email' ? (
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-muted-2 uppercase tracking-wider font-semibold">휴대폰 번호</label>
              <div className="relative">
              ...
              <p className="text-[11px] text-subtle">가입 시 등록한 휴대폰 번호를 입력해주세요.</p>
            </div>
          )}
          {error && (
          ...
          <button type="submit" disabled={loading}
            className="w-full h-11 flex items-center justify-center gap-2 bg-accent hover:bg-accent-strong disabled:opacity-50 text-white font-semibold rounded-xl transition-all shadow-[0_0_20px_rgba(139,92,246,0.25)]">
        ...
        <div className="space-y-4">
          <div className="bg-[#0a1520] border border-[#1a3040]/60 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2 text-accent-fg text-sm font-medium">
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1.5">
                  <User size={12} className="text-accent-fg" />
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-fg-3 text-sm font-mono">{result.email}</span>
              </div>
              {result.joinedAt && (
                <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={reset}
              className="flex-1 h-10 bg-surface border border-line hover:border-line-strong text-muted hover:text-fg-2 rounded-xl text-sm transition-all">
              다시 찾기
            </button>
            <button onClick={() => setTab('forgot-pw')}
              className="flex-1 h-10 flex items-center justify-center gap-1.5 bg-accent-tint border border-line-strong hover:bg-accent-tint text-accent-fg rounded-xl text-sm font-medium transition-all">
              <Lock size={12} /> 비밀번호 찾기
            </button>
          </div>
          <Link href="/login"
            className="w-full h-10 flex items-center justify-center gap-2 bg-accent hover:bg-accent-strong text-white rounded-xl text-sm font-semibold transition-all">
            로그인하기 <ArrowRight size={13} />
          </Link>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-start gap-2.5 text-sm text-muted bg-surface-2 border border-[#171625] rounded-xl px-4 py-4">
            <AlertCircle size={15} className="text-accent-2 shrink-0 mt-0.5" />
            <span>
              {method === 'email' ? '해당 이메일로' : '해당 휴대폰 번호로'} 가입된 계정을 찾을 수 없습니다.
            </span>
          </div>
          <button onClick={reset}
            className="w-full h-10 bg-surface border border-line hover:border-line-strong text-muted hover:text-fg-2 rounded-xl text-sm transition-all">
            다시 시도
          </button>
        </div>
      )}
    ...
    return (
      <div className="bg-surface border border-line rounded-2xl p-6 space-y-5">
        <div className="bg-emerald-950/40 border border-emerald-800/40 rounded-xl p-5 text-center space-y-2">
          ...
          <p className="text-sm text-fg-3">새 비밀번호로 로그인해주세요.</p>
        </div>
        <button onClick={() => router.push('/login')}
          className="w-full h-11 flex items-center justify-center gap-2 bg-accent hover:bg-accent-strong text-white font-semibold rounded-xl transition-all">
        ...
        {[{ label: '번호 입력', s: 'phone' }, { label: '인증 & 변경', s: 'otp' }].map(({ label, s }, i) => {
          ...
          return (
            <div key={s} className="flex items-center gap-0 flex-1">
              <div className={`flex items-center gap-1.5 text-xs font-medium ${isActive ? 'text-accent-fg' : isDone ? 'text-emerald-400' : 'text-[#201e32]'}`}>
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                  isActive ? 'bg-accent text-white' : isDone ? 'bg-emerald-600 text-white' : 'bg-line text-subtle'
                }`}>{isDone ? '✓' : i + 1}</span>
                {label}
              </div>
              {i === 0 && <div className="flex-1 h-px bg-line mx-2" />}
            </div>
          )
        })}
      </div>
      ...
      {step === 'phone' ? (
        <form onSubmit={requestOtp} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-subtle">가입 시 등록한 번호를 입력해주세요.</p>
          </div>
          {error && (
            <div className="flex items-center gap-2 text-sm text-red-400 bg-red-950/50 border border-red-800/40 rounded-xl px-4 py-3">
        ...
        <form onSubmit={verifyOtp} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xs text-muted-2 uppercase tracking-wider font-semibold">인증번호</label>
            ...
            <div className="flex gap-2 justify-between" onPaste={handleOtpPaste}>
              {otpDigits.map((digit, i) => (
                <input key={i} ref={setOtpRef(i)}
                  ...
                  className={`w-10 h-12 text-center text-lg font-bold bg-[#0b0a11] rounded-xl text-fg focus:outline-none transition-all border ${
                    digit
                      ? 'border-accent/70 shadow-[0_0_8px_rgba(139,92,246,0.15)]'
                      : 'border-line hover:border-line-strong focus:border-accent/50'
                  } ${otpExpired ? 'opacity-50' : ''}`}
                />
              ))}
            </div>
            <div className="flex justify-between items-center">
              <p className="text-[11px] text-subtle">{phone}으로 발송된 6자리 번호</p>
              <button type="button" onClick={resendOtp} disabled={resendCooldown > 0 || loading}
                className="flex items-center gap-1 text-[11px] transition-colors disabled:opacity-40 disabled:cursor-not-allowed text-muted-2 hover:text-accent-fg enabled:hover:text-accent-fg">
                <RefreshCw size={10} />
          ...
          <button type="submit" disabled={loading || otp.length !== 6 || !pwMatch || otpExpired}
            className="w-full h-11 flex items-center justify-center gap-2 bg-accent hover:bg-accent-strong disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all">
            {loading
              ? <span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />
