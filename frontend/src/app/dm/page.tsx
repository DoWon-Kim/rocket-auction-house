  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-fg">DM</h1>
        <Link href="/my/friends"
          className="flex items-center gap-1.5 text-xs text-accent-fg hover:text-accent-soft transition-colors">
          <Users size={13} /> 친구 관리
        </Link>
      </div>

      {isLoading && <p className="text-center text-subtle py-12 text-sm">로딩 중...</p>}

      {!isLoading && rooms.length === 0 && (
        <div className="text-center py-16">
          <MessageCircle size={36} className="mx-auto mb-3 text-subtle" />
          <p className="text-sm text-muted">아직 DM이 없습니다.</p>
          <p className="text-xs text-subtle mt-1">친구를 추가하면 DM을 시작할 수 있습니다.</p>
          <Link href="/my/friends" className="inline-block mt-4 text-xs text-accent-fg hover:text-accent-soft border border-accent/30 px-3 py-1.5 rounded-lg">
            친구 관리로 이동
          </Link>
        </div>
      )}
        ...
        {rooms.map(room => {
          ...
          return (
            <Link key={room.id} href={`/dm/${room.id}`}
              className="flex items-center gap-3 px-4 py-3 bg-sunken border border-line hover:border-line-strong rounded-xl transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between gap-2">
                  <p className={`text-sm font-semibold ${unread > 0 ? 'text-white' : 'text-fg-2'}`}>{partner.nickname}</p>
                  {room.lastMessageAt && (
                ...
                {lastMsg && (
                  <p className={`text-xs truncate mt-0.5 ${unread > 0 ? 'text-fg-2' : 'text-muted-2'}`}>
