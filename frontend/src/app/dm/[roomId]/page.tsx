...
export default function DmRoomPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-4 py-3 bg-sunken border-b border-line shrink-0">
        <button onClick={() => router.back()} className="text-muted hover:text-fg transition-colors p-1">
          <ArrowLeft size={18} />
        </button>
        {partner ? (
          <>
            <Avatar user={partner} />
            <div>
              <p className="text-sm font-semibold text-fg">{partner.nickname}</p>
              <p className="text-[10px] text-subtle">친구</p>
            </div>
          </>
        ) : (
        ...
        <div className={`ml-auto w-2 h-2 rounded-full ${connected ? 'bg-emerald-500' : 'bg-subtle'}`} title={connected ? '연결됨' : '연결 중...'} />
      </div>

      {/* 메시지 */}
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {messages.length === 0 && !room && (
          <div className="text-center py-16 text-subtle text-sm">로딩 중...</div>
        )}
        {messages.length === 0 && room && (
          <div className="text-center py-16 text-subtle text-sm">첫 메시지를 보내보세요!</div>
        )}
          ...
          {groupByDay(messages).map(group => (
            <div key={group.date}>
              <div className="flex items-center gap-3 my-4">
                ...
                <span className="text-[10px] text-subtle shrink-0">{group.date}</span>
                <div className="flex-1 h-px bg-line" />
              </div>
                    ...
                    <div key={msg.id} className={`flex items-end gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      <div className={`max-w-[70%] flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                        <div className={`px-3 py-2 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${isMine
                          ...
                          : 'bg-[#0a1020] border border-line text-fg-2 rounded-bl-sm'}`}>
                          {msg.content}
                        </div>
                        <div className="flex items-center gap-1 mt-0.5 px-1">
                          ...
                          {isMine && (
                            <CheckCheck size={11} className={msg.readAt ? 'text-accent-fg' : 'text-subtle'} />
                          )}
                        </div>
                      </div>
                    </div>
      ...
      {/* 입력창 */}
      <div className="shrink-0 px-4 py-3 bg-sunken border-t border-line">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 bg-surface border border-line focus:border-accent/40 rounded-xl px-4 py-2.5 text-sm text-fg placeholder:text-subtle focus:outline-none transition-colors resize-none"
          ...
          <button onClick={sendMsg} disabled={!input.trim() || !connected}
            className="w-10 h-10 rounded-xl bg-accent hover:bg-accent-strong disabled:opacity-40 flex items-center justify-center transition-colors shrink-0">
